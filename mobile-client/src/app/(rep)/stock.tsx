...
export default function RepStockScreen() {
  ...
  // This automatically stays synced with the Admin's live changes
  ...
  // Extract unique categories dynamically from live stock
  ...
  const handleCheckout = () => {
    Alert.alert(
      "Confirm Dispatch",
      `Checking out ETB ${cartTotal.toLocaleString()} worth of stock. This will be added to your ledger.`,
      [
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.bg }]}>
            ...
            <View key={item.id} style={[styles.productCard, { backgroundColor: theme.cardBg, borderColor: theme.border, borderWidth: isDarkMode ? 1 : 0 }, !isDarkMode && styles.lightShadow]}>
              ...
              <View style={styles.productDetails}>
                ...
                <Text style={[styles.stockText, { color: isOutOfStock ? '#DC2626' : theme.textMuted }]}>
                  {isOutOfStock ? 'Out of Stock' : `${item.stock} Available in Warehouse`}
                </Text>
              </View>

              {/* Upgraded Stepper with Manual Input */}
      ...
      <View style={[styles.checkoutFooter, { backgroundColor: theme.bg, borderTopColor: theme.border }]}>
        ...
        <TouchableOpacity 
          style={[styles.checkoutBtn, { backgroundColor: cartTotal > 0 ? theme.invertedBg : (isDarkMode ? '#2F3336' : '#E2E8F0') }]}
...
const styles = StyleSheet.create({
  ...
  checkoutBtn: { paddingHorizontal: 24, paddingVertical: 16, borderRadius: 100 }
});